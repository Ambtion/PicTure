...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if ([key isEqualToString:@"Cache"]) {
        if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImageinCache:)]) {
            [delegate performSelector:@selector(webImageManager:didFinishWithImageinCache:) withObject:self withObject:image];
        }
    }else{
